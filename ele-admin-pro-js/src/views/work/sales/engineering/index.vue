  ...
  <div class="ele-body">
    <vxe-modal v-model="demo1.value5" width="600" show-footer>
      ...
      <vxe-table
        ...
        <vxe-column field="engineeringName" title="工程名称"></vxe-column>
        <vxe-column field="unitid" title="客户ID"></vxe-column>
        <vxe-column field="engineeringAddress" title="工程所在地"></vxe-column>
        <vxe-column field="beginTime" title="开工时间"></vxe-column>
        <vxe-column field="enStatus" title="状态"></vxe-column>
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'unitName'">
            <router-link :to="'/work/sales/engineering/details?id=' + record.id">
              {{ record.unitName }}
            </router-link>
          </template>
...
import {
  ...
  removeGoods,
  updateUserStatus,
} from '@/api/work/sales/engineering';
...
let students = ref([{id: "001",rawMaterialId: "",rrId: 1,customerName:"", distance: 0, platformRawMaterials: "测试2",rmStatus: 1},
  {id: "002",rawMaterialId: "",rrId: 1,customerName:"", distance: 0, platformRawMaterials: "测试4",rmStatus: 1},
  {id: "003",rawMaterialId: "",rrId: 1,customerName:"", distance: 0, platformRawMaterials: "测试6",rmStatus: 1}])
// let students=ref([])
const selectAllGoods=async (record)=>{
  demo1.value5 = true
  // const res=await request.delete('/test/student/deleteStudent/'+row.id)
  const res=await request.get('/work/engineering/find/'+record.id);
...
const deleteEvent=async (row)=>{
  ...
  // axios.delete('http://localhost:8081/api/work/raw-material-supplier/deleteRR/'+row.rrId)

  const res=await request.delete('/work/engineering/deleteRR/'+row.rrId)
  console.log(res)
  if(res.data.code==0){
  ...
  // axios.delete('/work/raw-material-supplier/deleteRR/'+row.rrId)
  //   .then(response => {
